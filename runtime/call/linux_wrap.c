...
uintptr_t linux_get_memory_stats(size_t* heap_used, size_t* pages_available, size_t* total_memory, uint64_t* timestamp){
  size_t heap_size = 0, pages_avail = 0, total_mem = 0;
  uint64_t ts = 0;
  
  // 调用内存管理模块的函数
  get_memory_stats_with_time(&heap_size, &pages_avail, &total_mem, &ts);
  
  // 复制结果到用户空间
  if (heap_used) {
    copy_to_user(heap_used, &heap_size, sizeof(size_t));
  }
  if (pages_available) {
    copy_to_user(pages_available, &pages_avail, sizeof(size_t));
  }
  if (total_memory) {
    copy_to_user(total_memory, &total_mem, sizeof(size_t));
  }
  if (timestamp) {
    copy_to_user(timestamp, &ts, sizeof(uint64_t));
  }
  
  print_strace("[runtime] get_memory_stats: heap=%zu, avail=%zu, total=%zu, time=%llu\r\n", 
               heap_size, pages_avail, total_mem, ts);
