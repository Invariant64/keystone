...
int main(int argc, char* argv[]) {
      // 硬编码参数 ./cc -g 15 -n 1
    static int cc_argc_hardcoded = 5;
    static char* cc_argv_hardcoded[] = {
        (char*)"./cc",
        (char*)"-g", (char*)"15",
        (char*)"-n", (char*)"1",
        nullptr  // 结尾 null，不影响 CLApp，但有些库可能用到
    };
  argc = cc_argc_hardcoded; // Assign to the existing local variable
  argv = cc_argv_hardcoded; // Assign to the existing local variable
