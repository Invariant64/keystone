...
void load_data() {
    ...
    FILE *save = fopen(iris_data_filename, "w");
    if(!save) {
        printf("Couldn't open file: %s\n", iris_data_filename);
        exit(1);
    }
    fwrite(iris_data, 1, iris_data_len, save);
    fclose(save);


    /* Load the iris data-set. */
    FILE *in = fopen(iris_data_filename, "r");
    if (!in) {
